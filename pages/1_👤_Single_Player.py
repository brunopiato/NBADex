...
st.set_page_config(page_title='Single Player Vision',
...
selected_data_complete = data_complete[data_complete['PLAYER_NAME']
                                       == selected_player]
...
defensive_features = ['OREB', 'DREB', 'STL', 'BLK',
                      'CONTESTED_SHOTS', 'BOX_OUTS', 'CHARGES_DRAWN']
...
# CHARTS DATA
# Offensive features
aux_off = selected_data[selected_data['PLAYER_NAME']
                        == selected_player][offensive_features].T
aux_off.columns = [selected_player]
# Defensive features
aux_def = selected_data[selected_data['PLAYER_NAME']
                        == selected_player][defensive_features].T
aux_def.columns = [selected_player]
# Descriptive features
aux_desc = selected_data[selected_data['PLAYER_NAME']
                         == selected_player][descriptive_features].T
...
df_off["theta_radian"] = (
    df_off["theta_n"] / (df_off["theta_n"].max() + 1)) * 2 * np.pi
df_off["x"] = np.cos(df_off["theta_radian"]) * df_off["r"]
df_off["y"] = np.sin(df_off["theta_radian"]) * df_off["r"]
df_off_a = df_off.groupby("trace").apply(
    lambda d: sg.MultiPoint(list(zip(d["x"], d["y"]))).convex_hull.area)
fig_off = fig_off.for_each_trace(lambda t: t.update(
    name=f"{t.name} {df_off_a.loc[t.name]:.2f}"))
...
df_def["theta_radian"] = (
    df_def["theta_n"] / (df_def["theta_n"].max() + 1)) * 2 * np.pi
df_def["x"] = np.cos(df_def["theta_radian"]) * df_def["r"]
df_def["y"] = np.sin(df_def["theta_radian"]) * df_def["r"]
df_def_a = df_def.groupby("trace").apply(
    lambda d: sg.MultiPoint(list(zip(d["x"], d["y"]))).convex_hull.area)
fig_def = fig_def.for_each_trace(lambda t: t.update(
    name=f"{t.name} {df_def_a.loc[t.name]:.2f}"))
...
df_desc["theta_radian"] = (
    df_desc["theta_n"] / (df_desc["theta_n"].max() + 1)) * 2 * np.pi
df_desc["x"] = np.cos(df_desc["theta_radian"]) * df_desc["r"]
df_desc["y"] = np.sin(df_desc["theta_radian"]) * df_desc["r"]
df_desc_a = df_desc.groupby("trace").apply(
    lambda d: sg.MultiPoint(list(zip(d["x"], d["y"]))).convex_hull.area)
fig_desc = fig_desc.for_each_trace(lambda t: t.update(
    name=f"{t.name} {df_desc_a.loc[t.name]:.2f}"))

# Getting data from the API
player_shotchart_df, league_avg = funcoes.get_player_shotchartdetail(
    player_name=selected_player, season_id='2022-23')
...
with st.container():
    ...
    with col4:
        ...
        fig_shotchart = funcoes.shot_chart(player_shotchart_df,
                                           title="{[0]}".format(
                                               player_shotchart_df['PLAYER_NAME']),
                                           flip_court=False,
                                           court_lw=2)
